...
def optimize_scenario_TS_SO(filename,N,P,PM,node2dm,p,same,dm,popdm,S0,I0,bg,cap,cB,cPM,mode):
    t0 = time.time()
    print(filename)
    theta_true,theta_env,theta_hat,T,eps = read_scenario(filename,dm,N)
    S,I,R,D,a,b,xub,ratio,lbd = initialization(N,node2dm,popdm,S0,I0)
    ...
    for t in range(1,T+1):
        ...
        #---select action
        x = optimize_SO(t,xub,theta_hat[t],N,dm,S,I,p,node2dm,bg,ratio,P,B)
        for i in N:
            xall[t][i] = x[i]
            xub[i] = 1.0 - sum(xall[tt][i] for tt in range(max(t-PM[i]+1,1),t+1))
